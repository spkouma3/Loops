...
def Ell(_r,_na,_slp):
    return _r-nlp(_r,_na,_slp)*(_slp-1)
...
def logRF(_r,_na,_slp):
    s=0
    splus=0
    narange=np.arange(1,_na+1)
    #if you print youll see arange only gives one less
    naplusone=np.arange(1,_na+2)
    for j in narange:
        s+=logw(j,_na,_r,_slp)
    for j in naplusone:
        splus+=logw(j,_na,_r,_slp)
    return splus\
        -\
        s\
        -\
...
def logRlp(_na,_r,_slp):
    return (_na+1)*\
    ScrSlp(_na+1,_r,_slp)\
    -_na*ScrSlp(_na,_r,_slp)\
    +(_r/(_slp-1))*\
    (\
    np.log(mu3)\
    +np.log(B3)\
...
def DeltaQlog(_na,_r,_slp,_nvol,_n):
    return logRF(_r,_na,_slp)
    +\
    logRlp(_na,_r,_slp)\
    +\
    logRvol(_na,_nvol,_n)